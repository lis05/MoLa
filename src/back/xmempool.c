...
static uint32_t         _xmem_pool_next_alloc_block_node_count = MIN_ALLOC_BLOCK_NODE_COUNT;
...
#pragma GCC diagnostic ignored "-Wpointer-to-int-cast"
        printf("  + spaces: [0x%.8X, 0x%.8X)\n", (unsigned int)pool->start, (unsigned int)pool->end);
...
char *xmem_alloc(xmem_pool_handle handle) {
    ...
    if (!pool->free_blocks) {
        xmem_pool *new_pool = (xmem_pool *)_create_pool(pool->block_size, pool->next_alloc_length);
